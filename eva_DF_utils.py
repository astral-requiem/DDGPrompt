...
from utils.utils import set_random_seed, contrastive_loss
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','DF2','DF']:
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                      dst_node_ids=batch_neg_dst_node_ids,
                                                                      node_interact_times=batch_node_interact_times)
            elif model_name in ['DF']:
                # (b, l, 4 * emb_dim)
                patches_data = model[0].get_src_des_node_combine_fea(batch_src_node_ids,batch_dst_node_ids,batch_node_interact_times)
                # adaptive neighbour num prompt
                # (b, l, 4 * emb_dim)   input embedding to mamba
                patches_data = model[1].add_mlp_weight_prompt(patches_data)
                # get temporal embedding of source and destination nodes
                # Tensor, shape (batch_size, node_feat_dim=172)
                batch_src_node_embeddings, batch_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(patches_data)
                # downstream task prompt
                # Tensor, shape (batch_size, node_feat_dim=172)
                batch_src_node_embeddings, batch_dst_node_embeddings = model[1].add_feature_prompt(batch_src_node_embeddings, batch_dst_node_embeddings,patches_data)


                # (b, l, 4 * emb_dim)
                patches_data = model[0].get_src_des_node_combine_fea(batch_neg_src_node_ids, batch_neg_dst_node_ids,batch_node_interact_times)
                # adaptive neighbour num prompt
                # (b, l, 4 * emb_dim)   input embedding to mamba
                patches_data = model[1].add_mlp_weight_prompt(patches_data)
                # get temporal embedding of negative source and negative destination nodes
                # Tensor, shape (batch_size, node_feat_dim=172)
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(patches_data)
                # downstream task prompt
                # Tensor, shape (batch_size, node_feat_dim=172)
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model[1].add_feature_prompt(batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings,patches_data)
            else:
                raise ValueError(f"Wrong value for model_name {model_name}!")
            # are_equal = torch.equal(batch_src_node_embeddings, batch_neg_src_node_embeddings)
            # print(are_equal)
            # get positive and negative probabilities, shape (batch_size, )
            positive_probabilities = model[2](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
            negative_probabilities = model[2](input_1=batch_neg_src_node_embeddings, input_2=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()

            predicts = torch.cat([positive_probabilities, negative_probabilities], dim=0)
            labels = torch.cat([torch.ones_like(positive_probabilities), torch.zeros_like(negative_probabilities)], dim=0)

            loss = loss_func(input=predicts, target=labels)
            # loss = contrastive_loss(batch_src_node_embeddings, batch_dst_node_embeddings, batch_neg_dst_node_embeddings,
            #                         tau=0.1)
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                # (b, l, 4 * emb_dim)
                # src_nodes_combine_fea, dst_nodes_combine_fea = model[0].get_src_des_node_combine_fea(batch_src_node_ids,batch_dst_node_ids,batch_node_interact_times)
                patches_data = model[0].get_src_des_node_combine_fea(batch_src_node_ids, batch_dst_node_ids,
                                                                     batch_node_interact_times)
                # adaptive neighbour num prompt
                # (b, l, 4 * emb_dim)   input embedding to mamba
                # src_nodes_combine_fea, dst_nodes_combine_fea = model[1].add_mlp_weight_prompt(src_nodes_combine_fea,dst_nodes_combine_fea)
                # patches_data = model[0].get_middle_embedding(patches_data)
                patches_data = model[1].add_mlp_weight_prompt(patches_data)
                # get temporal embedding of source and destination nodes
                # Tensor, shape (batch_size, node_feat_dim=172)
                # batch_src_node_embeddings, batch_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(src_nodes_combine_fea, dst_nodes_combine_fea)
                batch_src_node_embeddings, batch_dst_node_embeddings = model[0].compute_src_dst_node_temporal_embeddings(patches_data)
                # downstream task prompt
                # Tensor, shape (batch_size, node_feat_dim=172)
                batch_src_node_embeddings, batch_dst_node_embeddings = model[1].add_feature_prompt(batch_src_node_embeddings, batch_dst_node_embeddings,patches_data)
