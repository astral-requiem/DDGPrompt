...
class DF2(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        #     MambaBlock(input_dim = self.num_channels * self.channel_embedding_dim) for _ in range(self.mambaLayers)
        # ])
        ...
        self.output_layer = nn.Linear(in_features=self.num_channels * self.channel_embedding_dim, out_features=self.edge_feat_dim, bias=True)
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
        ...
        for idx in range(len(nodes_neighbor_ids_list)):
            ...
            if len(nodes_neighbor_ids_list[idx]) > max_input_sequence_length - 1:
                # 对超过max_input_sequence_length的一阶邻域节点做最近交互的截断，丢弃比较早的节点交互信息，但是不足max_input_sequence_length的还没有padding
