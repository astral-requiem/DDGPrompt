...
class TCL(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_neighbor_delta_times = node_interact_times[:, np.newaxis] - src_neighbor_times
        dst_neighbor_delta_times = node_interact_times[:, np.newaxis] - dst_neighbor_times
    ...
    def get_all_features(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray, num_neighbors: int = 20):
        """
        compute source and destination node temporal embeddings
        :param src_node_ids: ndarray, shape (batch_size, )
        ...
        return src_nodes_neighbor_node_raw_features, src_nodes_edge_raw_features, src_nodes_neighbor_time_features, src_nodes_neighbor_depth_features, \
               dst_nodes_neighbor_node_raw_features, dst_nodes_edge_raw_features, dst_nodes_neighbor_time_features, dst_nodes_neighbor_depth_features, neighbor_delta_times,\
               src_neighbor_node_ids,dst_neighbor_node_ids

    def compute_src_dst_node_temporal_embeddings_dyg(self, src_nodes_neighbor_node_raw_features, src_nodes_edge_raw_features, src_nodes_neighbor_time_features, src_nodes_neighbor_depth_features, \
               dst_nodes_neighbor_node_raw_features, dst_nodes_edge_raw_features, dst_nodes_neighbor_time_features, dst_nodes_neighbor_depth_features, neighbor_delta_times,src_neighbor_node_ids,dst_neighbor_node_ids):
    ...
    def get_src_des_data(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray, num_neighbors: int = 20):
        """
        compute source and destination node temporal embeddings
        :param src_node_ids: ndarray, shape (batch_size, )
        ...
        bs = src_nodes_neighbor_node_raw_features.shape[0]

        src_data = torch.cat((src_nodes_neighbor_node_raw_features,src_nodes_edge_raw_features,
                              src_nodes_neighbor_time_features,src_nodes_neighbor_depth_features.unsqueeze(0).expand(bs, -1, -1)),dim=-1)
        dst_data = torch.cat((dst_nodes_neighbor_node_raw_features, dst_nodes_edge_raw_features,
                              dst_nodes_neighbor_time_features, dst_nodes_neighbor_depth_features.unsqueeze(0).expand(bs, -1, -1)), dim=-1)

        return src_data,dst_data,src_neighbor_node_ids,dst_neighbor_node_ids,src_neighbor_times,dst_neighbor_times

    def compute_embedding(self,src_data,dst_data,src_neighbor_node_ids,dst_neighbor_node_ids):

        bs,len,four_dim = src_data.shape
        dim = four_dim // 4

        src_nodes_neighbor_node_raw_features = src_data[:,:,:dim]
        src_nodes_edge_raw_features = src_data[:,:,dim:2*dim]
        src_nodes_neighbor_time_features = src_data[:,:,2*dim:3*dim]
        src_nodes_neighbor_depth_features = src_data[:,:,3*dim:4*dim]

        dst_nodes_neighbor_node_raw_features = dst_data[:, :, :dim]
        dst_nodes_edge_raw_features = dst_data[:, :, dim:2 * dim]
        dst_nodes_neighbor_time_features = dst_data[:, :, 2 * dim:3 * dim]
        dst_nodes_neighbor_depth_features = dst_data[:, :, 3 * dim:4 * dim]
