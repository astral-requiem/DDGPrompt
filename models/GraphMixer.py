...
class GraphMixer(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_embeddings,src_neighbor_delta_times = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings,dst_neighbor_delta_times = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap)

        neighbor_delta_times = np.concatenate((src_neighbor_delta_times,dst_neighbor_delta_times),axis=1)

        return src_node_embeddings, dst_node_embeddings,neighbor_delta_times
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        nodes_neighbor_time_features = self.time_encoder(timestamps=torch.from_numpy(neighbor_delta_times).float().to(self.device))
        ...
        return node_embeddings,neighbor_delta_times
    ...
    def compute_src_dst_node_temporal_embeddings_dyg(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray, num_neighbors: int = 20, time_gap: int = 2000):
        """
        compute source and destination node temporal embeddings
        :param src_node_ids: ndarray, shape (batch_size, )
        ...
        src_time_gap_neighbor_node_ids,src_nodes_time_gap_neighbor_node_raw_features,src_node_feature = self.get_all_features(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap)
        dst_nodes_edge_raw_features, dst_nodes_neighbor_time_features, \
        dst_time_gap_neighbor_node_ids, dst_nodes_time_gap_neighbor_node_raw_features,dst_node_feature = self.get_all_features(
            node_ids=dst_node_ids, node_interact_times=node_interact_times,
            num_neighbors=num_neighbors, time_gap=time_gap)

        return src_nodes_edge_raw_features,src_nodes_neighbor_time_features,\
    ...
    def compute_src_dst_node_temporal_embeddings_dyg_(self, src_nodes_edge_raw_features,src_nodes_neighbor_time_features,\
        src_time_gap_neighbor_node_ids,src_nodes_time_gap_neighbor_node_raw_features,src_node_feature,
        dst_nodes_edge_raw_features, dst_nodes_neighbor_time_features, \
        dst_time_gap_neighbor_node_ids, dst_nodes_time_gap_neighbor_node_raw_features,dst_node_feature,src_node_ids,dst_node_ids):


        src_node_embeddings = self.compute_node_temporal_embeddings_dyg(src_node_ids,src_nodes_edge_raw_features,src_nodes_neighbor_time_features,\
        src_time_gap_neighbor_node_ids,src_nodes_time_gap_neighbor_node_raw_features,src_node_feature)

        dst_node_embeddings = self.compute_node_temporal_embeddings_dyg(dst_node_ids, dst_nodes_edge_raw_features,
                                                                        dst_nodes_neighbor_time_features, \
                                                                        dst_time_gap_neighbor_node_ids,
                                                                        dst_nodes_time_gap_neighbor_node_raw_features,
                                                                        dst_node_feature)

        return src_node_embeddings, dst_node_embeddings


    def get_all_features(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                         num_neighbors: int = 20, time_gap: int = 2000):
        """
        given node ids node_ids, and the corresponding time node_interact_times, return the temporal embeddings of nodes in node_ids
        :param node_ids: ndarray, shape (batch_size, ), node ids
        ...
        node_feature = self.node_raw_features[torch.from_numpy(node_ids)]

        #--------------------------

        return nodes_edge_raw_features,nodes_neighbor_time_features,time_gap_neighbor_node_ids,nodes_time_gap_neighbor_node_raw_features,node_feature

    def compute_node_temporal_embeddings_dyg(self,node_ids,nodes_edge_raw_features,nodes_neighbor_time_features,time_gap_neighbor_node_ids,nodes_time_gap_neighbor_node_raw_features,node_feature):
        ...
        # Tensor, shape (batch_size, node_feat_dim), add features of nodes in node_ids
        output_node_features = nodes_time_gap_neighbor_node_agg_features + node_feature
    ...
    def get_datas(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                        node_interact_times: np.ndarray, num_neighbors: int = 20, time_gap: int = 2000):

        src_data,src_neighbor_times = self.get_data(node_ids=src_node_ids,node_interact_times=node_interact_times,num_neighbors=num_neighbors,
                                                                                              time_gap=time_gap)
        dst_data,dst_neighbor_times = self.get_data(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                 num_neighbors=num_neighbors,
                                 time_gap=time_gap)
        return src_data,dst_data,src_neighbor_times,dst_neighbor_times

    def get_data(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                         num_neighbors: int = 20, time_gap: int = 2000):
        """
        given node ids node_ids, and the corresponding time node_interact_times, return the temporal embeddings of nodes in node_ids
        :param node_ids: ndarray, shape (batch_size, ), node ids
        ...
        return combined_features,neighbor_times

    def compute_embeddings(self,src_combined_features,dst_combined_features,src_node_ids,dst_node_ids,node_interact_times,time_gap):
        src_node_embeddings = self.compute_embedding(combined_features=src_combined_features,node_ids=src_node_ids,node_interact_times=node_interact_times,
                                                     time_gap=time_gap)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings = self.compute_embedding(combined_features=dst_combined_features,node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                     time_gap=time_gap)
        return src_node_embeddings, dst_node_embeddings
    def compute_embedding(self,combined_features,node_ids,node_interact_times,time_gap):
