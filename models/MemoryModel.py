...
from models.prompt import dyg_prompt
...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 dst_node_std_time_shift: float = 1.0, device: str = 'cpu',use_dyg_prompt:bool = False):
        """
        ...
        self.use_dyg_prompt = use_dyg_prompt
        ...
        if self.model_name == 'JODIE':
            ...
            self.embedding_module = GraphAttentionEmbedding(node_raw_features=self.node_raw_features,
                                                            ...
                                                            use_dyg_prompt=self.use_dyg_prompt)
        else:
            raise ValueError(f'Not implemented error for model_name {self.model_name}!')
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if self.model_name == 'JODIE':
            ...
            node_embeddings, neighbor_delta_times = self.embedding_module.compute_node_temporal_embeddings(node_memories=updated_node_memories,
                                                                                     node_ids=node_ids,
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def __init__(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor, neighbor_sampler: NeighborSampler,
                 ...
                 num_layers: int = 2, num_heads: int = 2, dropout: float = 0.1,use_dyg_prompt:bool = True):
        """
        ...
        self.merge_layers = nn.ModuleList([MergeLayer(input_dim1=self.node_feat_dim + self.time_feat_dim, input_dim2=self.node_feat_dim,
                                                      hidden_dim=self.node_feat_dim, output_dim=self.edge_feat_dim) for _ in range(num_layers)])

        self.use_dyg_prompt = use_dyg_prompt

        self.dyg_prompt = dyg_prompt(node_feat_dim = edge_feat_dim,time_feat_dim= time_feat_dim,alpha = 2)

        for param in self.dyg_prompt.parameters():
            param.requires_grad = self.use_dyg_prompt
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            node_conv_features = self.compute_node_temporal_embeddings(node_memories=node_memories,
                                                                       ...
                                                                       num_neighbors=num_neighbors)
            if self.use_dyg_prompt:
                node_conv_features, node_time_features = self.dyg_prompt.add_dyg_prompt(node_conv_features,node_time_features)
            ...
            if self.use_dyg_prompt:
                neighbor_node_conv_features, neighbor_time_features = self.dyg_prompt.add_dyg_prompt(neighbor_node_conv_features,neighbor_time_features)
            ...
            return output,neighbor_delta_times
