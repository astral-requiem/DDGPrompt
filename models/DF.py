...
class DF(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, embedding_dim: int, patch_size:int,num_layers: int = 2, dropout: float = 0.2, num_neighbors: int = 256, device: str = 'cpu'):
        """
        ...
        # self.patch_size = 2     # reddit
        # self.patch_size = 8      # mooc
        # self.patch_size = 16

        self.time_encoder = TimeEncoder(time_dim=time_feat_dim,parameter_requires_grad=False)

        self.neighbor_co_occurrence_feat_dim = self.embedding_dim
        self.neighbor_co_occurrence_encoder = NeighborCooccurrenceEncoder(neighbor_co_occurrence_feat_dim=50, device=self.device,
                                                                          node_raw_features=self.node_raw_features)


        self.projection_layer = nn.ModuleDict({
            'node': nn.Linear(in_features=self.patch_size * self.node_feat_dim, out_features=self.embedding_dim,
                              bias=True),
            'edge': nn.Linear(in_features=self.patch_size * self.edge_feat_dim, out_features=self.embedding_dim,
                              bias=True),
            'time': nn.Linear(in_features=self.patch_size * self.time_feat_dim, out_features=self.embedding_dim,
                              bias=True),
            'neighbor_co_occurrence': nn.Linear(in_features=self.patch_size * self.neighbor_co_occurrence_feat_dim,
                                                out_features=self.embedding_dim, bias=True),
        })
        self.num_channels = 4

        self.transformers = nn.ModuleList([
            TransformerEncoder(attention_dim=self.num_channels * self.embedding_dim, num_heads=2,
                               dropout=self.dropout)
            for _ in range(self.layers)
        ])

        self.output_layer = nn.Linear(in_features=self.num_channels * self.embedding_dim, out_features=self.edge_feat_dim, bias=True)

    def compute_src_dst_node_temporal_embeddings(self, patches_data,src_num_patches):           # (bs,2l,4d)


        for transformer in self.transformers:
            patches_data = transformer(patches_data)

        src_nodes_combine_fea = patches_data[:,:src_num_patches,:]
        dst_nodes_combine_fea = patches_data[:,src_num_patches:,:]

        # Tensor, shape (batch_size, 4 * embedding_dim)
        src_nodes_combine_fea = torch.mean(src_nodes_combine_fea, dim=1)
        dst_nodes_combine_fea = torch.mean(dst_nodes_combine_fea, dim=1)

        # Tensor, shape (batch_size, node_feat_dim=172)
        src_node_embeddings = self.output_layer(src_nodes_combine_fea)
        dst_node_embeddings = self.output_layer(dst_nodes_combine_fea)
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
                      nodes_neighbor_times_list: list, patch_size: int = 1, max_input_sequence_length: int = 256,n_list: list = [1]):
        """
    ...
    def get_features(self, node_ids, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        # padded_nodes_neighbor_time_features = time_encoder(timestamps=torch.from_numpy(node_interact_times[:, np.newaxis, np.newaxis] - padded_nodes_neighbor_times).float().to(self.device))
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: torch.Tensor, padded_nodes_edge_raw_features: torch.Tensor,
        ...
        # patches_nodes_neighbor_node_raw_features = torch.stack(patches_nodes_neighbor_node_raw_features, dim=1).reshape(batch_size, num_patches, 2,patch_size * self.node_feat_dim)
    ...
    def get_src_des_node_combine_fea(self,src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        # src_padded_nodes_neighbor_ids, ndarray, shape (batch_size,k,  num_neighbors)
        # src_padded_nodes_edge_ids, ndarray, shape (batch_size,k,  num_neighbors)
        # src_padded_nodes_neighbor_times, ndarray, shape (batch_size,k,  num_neighbors)
        src_padded_nodes_neighbor_ids, src_padded_nodes_edge_ids, src_padded_nodes_neighbor_times = \
            self.pad_sequences(node_ids=src_node_ids, node_interact_times=node_interact_times,
                               ...
                               patch_size=self.patch_size, max_input_sequence_length=self.num_neighbors)

        # dst_padded_nodes_neighbor_ids, ndarray, shape (batch_size,k, num_neighbors)
        # dst_padded_nodes_edge_ids, ndarray, shape (batch_size,k,  num_neighbors)
        # dst_padded_nodes_neighbor_times, ndarray, shape (batch_size,k,  num_neighbors)
        ...
        neighbor_time = np.concatenate((src_padded_nodes_neighbor_times,dst_padded_nodes_neighbor_times),axis=1)

        # get the features of the sequence of source and destination nodes
        # src_padded_nodes_neighbor_node_raw_features, Tensor, shape (batch_size, num_neighbors, node_feat_dim)
        # src_padded_nodes_edge_raw_features, Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
        # src_padded_nodes_neighbor_time_features, Tensor, shape (batch_size, num_neighbors, time_feat_dim)
        src_padded_nodes_neighbor_node_raw_features, src_padded_nodes_edge_raw_features, src_padded_nodes_neighbor_time_features = \
            self.get_features(node_ids=src_node_ids,
                              node_interact_times=node_interact_times,
        ...
        # dst_padded_nodes_neighbor_node_raw_features, Tensor, shape (batch_size, num_neighbors, node_feat_dim)
        # dst_padded_nodes_edge_raw_features, Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
        # dst_padded_nodes_neighbor_time_features, Tensor, shape (batch_size, num_neighbors, time_feat_dim)
        dst_padded_nodes_neighbor_node_raw_features, dst_padded_nodes_edge_raw_features, dst_padded_nodes_neighbor_time_features = \
            self.get_features(node_ids=dst_node_ids,
                              node_interact_times=node_interact_times,
        ...
        # Tensor, shape (batch_size, len, embedding_dim)
        src_nodes_neighbor_node_projection_features = self.projection_layer['node'](
            src_patches_nodes_neighbor_node_raw_features)
        src_nodes_edge_projection_features = self.projection_layer['edge'](src_patches_nodes_edge_raw_features)
        src_nodes_neighbor_time_projection_features = self.projection_layer['time'](
        ...
        # Tensor, shape (batch_size, len, embedding_dim)
        dst_nodes_neighbor_node_projection_features = self.projection_layer['node'](
            dst_patches_nodes_neighbor_node_raw_features)
        dst_nodes_edge_projection_features = self.projection_layer['edge'](dst_patches_nodes_edge_raw_features)
        dst_nodes_neighbor_time_projection_features = self.projection_layer['time'](
        ...
        batch_size = len(src_nodes_neighbor_node_projection_features)
        ...
        patches_nodes_neighbor_node_raw_features = torch.cat(
            [src_nodes_neighbor_node_projection_features, dst_nodes_neighbor_node_projection_features], dim=1)
        patches_nodes_edge_raw_features = torch.cat(
            [src_nodes_edge_projection_features, dst_nodes_edge_projection_features], dim=1)
        patches_nodes_neighbor_time_features = torch.cat(
            [src_nodes_neighbor_time_projection_features, dst_nodes_neighbor_time_projection_features], dim=1)
        ...
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim),[200,64,200][bt,seq,fea_dim]
        patches_data = patches_data.reshape(batch_size, src_num_patches + dst_num_patches, self.num_channels * self.embedding_dim)


        return patches_data,neighbor_time,src_num_patches
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def __init__(self, neighbor_co_occurrence_feat_dim: int, device: str = 'cpu',node_raw_features=None):
        """
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        for src_padded_node_neighbor_ids, dst_padded_node_neighbor_ids in zip(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids):
            ...
            # # 获取源节点的二进制编码特征
            # src_binary_encodings = torch.tensor(self.node_raw_features[src_padded_node_neighbor_ids],
            #                                     dtype=torch.float32).to(self.device)
            #
            # # 获取目标节点的二进制编码特征
            # dst_binary_encodings = torch.tensor(self.node_raw_features[dst_padded_node_neighbor_ids],
            #                                     dtype=torch.float32).to(self.device)
            ...
            # src_padded_nodes_appearances.append(torch.cat((src_padded_node_neighbor_counts_in_src.unsqueeze(-1),src_binary_encodings),dim=-1))
            ...
            # dst_padded_nodes_appearances.append(dst_padded_node_neighbor_counts_in_dst)
            # dst_padded_nodes_appearances.append(torch.cat((dst_padded_node_neighbor_counts_in_dst.unsqueeze(-1),dst_binary_encodings),dim=-1))
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        # src_padded_nodes_neighbor_ids = np.squeeze(src_padded_nodes_neighbor_ids)
        # dst_padded_nodes_neighbor_ids = np.squeeze(dst_padded_nodes_neighbor_ids)
        ...
        src_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer(src_padded_nodes_appearances).sum(dim=2)
        ...
        # dst_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer(dst_padded_nodes_appearances.unsqueeze(dim=-1))
        ...
        # src_padded_nodes_neighbor_co_occurrence_features = torch.unsqueeze(src_padded_nodes_neighbor_co_occurrence_features,dim=1)
        # dst_padded_nodes_neighbor_co_occurrence_features = torch.unsqueeze(dst_padded_nodes_neighbor_co_occurrence_features,dim=1)
