...
# from DyGLib.few_shot_link_pre import l2_regularization
...
from utils.DataLoader import get_idx_data_loader, get_link_prediction_data, load_all_spilt_data, \
    generate_few_shot_dataset, split_train_data,get_node_classification_data,get_few_shot_node_classification_data
from utils.EarlyStopping import EarlyStopping
from utils.load_configs import get_link_prediction_args,get_node_classification_args
...
def process_datasets(full_data: Data, pretrain_data: Data, finetune_data: Data,
                     val_data: Data, test_data: Data, max_interactions: int) -> tuple:
    # 计算每个数据集的交互次数
    ...
    return (
        ...
        filtered_val_data, filtered_test_data,)
...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, full_data, pretrain_data, finetune_data, val_data, test_data = \
        get_few_shot_node_classification_data(dataset_name=args.dataset_name, pretrain_ratio=0.8,finetune_size=70)

    # full_data, pretrain_data, finetune_data, val_data, test_data = \
    #     process_datasets(full_data, pretrain_data, finetune_data, val_data, test_data, 100)

    train_data = finetune_data          # 1% few-shot train dataset

    # train_data = [train_data]
    # val_data = [val_data]
    #
    # train_data = generate_few_shot_dataset(train_data, args.train_shotnum, args.label_num, args.few_shot_tasknum,
    #                                      args.seed, task_type="node")
    # val_data = generate_few_shot_dataset(val_data, args.val_shotnum, args.label_num, args.few_shot_tasknum,
    #                                        args.seed, task_type="node")
    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = DF2(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=full_neighbor_sampler,
                                         time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim, patch_size=args.patch_size,
                                         num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                         max_input_sequence_length=args.max_input_sequence_length, device=args.device)
        elif args.model_name == 'DF':
            dynamic_backbone = DF(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=full_neighbor_sampler,
        ...
        node_classifier = MLPClassifier(input_dim=edge_raw_features.shape[1], dropout=args.dropout)
        ...
        if args.model_name == "DF" or args.use_ddg_prompt:
            # prompt = Prompt(time_feat_dim = args.time_feat_dim,channel_embedding_dim=args.channel_embedding_dim,num_channels=4,hidden_dim=16)
            # TCL
            prompt = Prompt(time_feat_dim = args.time_feat_dim,channel_embedding_dim=50,num_channels=4,hidden_dim=8)
            ...
            model = nn.Sequential(model, prompt, node_classifier)
            ...
            if args.tig_prompt:
                ...
                model = nn.Sequential(model, tig_Prompt, node_classifier)
                ...
                model = nn.Sequential(model, dygprompt, node_classifier)
            else:
                model = nn.Sequential(model, node_classifier)
