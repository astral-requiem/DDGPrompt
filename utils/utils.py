...
class NeighborSampler:
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            # find neighbors that interacted with node_id before time node_interact_time
            # 函数找到了截止当前node_interact_times全部的节点一阶邻居，还没有做截断
...
def contrastive_loss(emb_i, emb_j, emb_neg_i, emb_neg_j, tau=0.2):

    # emb_i = torch.nn.functional.normalize(emb_i, p=2, dim=1)
    # emb_j = torch.nn.functional.normalize(emb_j, p=2, dim=1)
    # # emb_neg_i = torch.nn.functional.normalize(emb_neg_i, p=2, dim=1)
    # emb_neg_j = torch.nn.functional.normalize(emb_neg_j, p=2, dim=1)

    # # 计算正样本对的相似度
    # pos_sim = torch.sum(emb_i * emb_j, dim=-1)  # 点积，假设向量已经归一化
    #
    # # 计算负样本对的相似度
    # neg_sim = torch.sum(emb_i * emb_neg_j, dim=-1)  # 与负样本的点积
    # # neg_sim = torch.sum(emb_neg_i * emb_neg_j, dim=-1)  # 与负样本的点积
    #
    # # 计算损失
    # # pos_term = pos_sim / tau
    # pos_term = torch.exp(pos_sim / tau)
    # neg_term = torch.exp(neg_sim / tau)
    #
    # # loss = -torch.log( pos_term /  neg_term)
    # loss = -torch.log( pos_term / (pos_term + neg_term))
    #
    # return loss.mean()

    tau = torch.tensor(tau, dtype=float)

    loss = -1 * torch.log(
        torch.exp(F.cosine_similarity(emb_i, emb_j) / tau) /
        (torch.exp(F.cosine_similarity(emb_i, emb_j) / tau) + torch.exp(F.cosine_similarity(emb_i, emb_neg_j) / tau))
    )
